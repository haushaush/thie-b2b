...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { requestId, userEmail, companyName, contactPerson, message, subject } = payload;
...
<title>Bestellinformation - Thie B2B Portal</title>
</head>
...
<h2 style="margin: 0 0 20px; font-size: 20px; font-weight: 600; color: #1a1a1a;">Nachricht zu Ihrer Bestellung #${shortId}</h2>

<p style="margin: 0 0 16px; font-size: 15px; line-height: 1.6; color: #333333;">Hallo ${customerName},</p>

<p style="margin: 0 0 24px; font-size: 15px; line-height: 1.6; color: #333333;">wir haben eine Nachricht zu Ihrer Bestellung für Sie:</p>

<!-- Message Box -->
<div style="background-color: #f7faf9; border: 1px solid #e0ebe7; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
<p style="margin: 0 0 12px; font-size: 14px; font-weight: 600; color: #009c77; text-transform: uppercase; letter-spacing: 0.5px;">Nachricht unseres Teams:</p>
...
<p style="margin: 0; font-size: 14px; color: #666666;">Bestellnummer: <strong style="color: #333333;">#${shortId}</strong></p>
</div>

<p style="margin: 0 0 28px; font-size: 15px; line-height: 1.6; color: #333333;">Sie können den aktuellen Status Ihrer Bestellung jederzeit in unserem Portal einsehen. Bei Rückfragen stehen wir Ihnen gerne zur Verfügung.</p>

<!-- CTA Button -->
<table role="presentation" cellspacing="0" cellpadding="0" style="margin: 0 auto 28px;">
<tr>
<td style="border-radius: 8px; background-color: #009c77;">
<a href="${baseUrl}/requests" style="display: inline-block; padding: 14px 32px; font-size: 15px; font-weight: 600; color: #ffffff; text-decoration: none;">Bestellung im Portal ansehen</a>
</td>
</tr>
</table>
...
<a href="${baseUrl}/privacy" style="color: #bbbbbb; text-decoration: none;">Datenschutz</a>
    ...
    console.log("Order message email sent to:", userEmail);
    ...
    console.error("Error in send-order-message:", error);
